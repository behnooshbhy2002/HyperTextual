...
from hyper.tasks import build_weighted_graph



class Hypergraph_Dataset:
    """
    ...
    To obtain it, we get a dataset object with dict:
        dataset.data["train"] -> training data, an torch array with shape (num_triples, max_arity + 1) of form (rel, e1, e2, e3, e4, e5, e6)
        dataset.data["test"] -> test data, an torch array with shape (num_triples, max_arity + 1) of form (rel, e1, e2, e3, e4, e5, e6)
        dataset.data["valid"] -> validation data, an torch array with shape (num_triples, max_arity + 1) of form (rel, e1, e2, e3, e4, e5, e6)
    """
    def __init__(self, ds_name, root, device="cpu", binary_dataset = False, inductive_dataset = False):
        self.name = ds_name
        self.device = device
        ...
        self.dataclass["train_edge"], self.dataclass["train_rel"] = self.read(os.path.join(self.dir, "train.txt"))

        # compute the average degree of the dataset
        self.density = len(self.dataclass["train_edge"]) / (len(self.ent2id) )
    

        # Load the test data
        self.dataclass["test_edge"], self.dataclass["test_rel"] = self.read_test(os.path.join(self.dir, "test.txt"))
        ...
        if self.name == "JF17K":
            for i in range(2,7):
                ...
                self.dataclass[test_arity+"_edge"], self.dataclass[test_arity+"_rel"] = self.read_test(file_path)
                
        self.dataclass["valid_edge"], self.dataclass["valid_rel"] = self.read(os.path.join(self.dir, "valid.txt"))
        
        if self.inductive_dataset:
            self.dataclass["aux_edge"], self.dataclass["aux_rel"] = self.read(os.path.join(self.dir, "aux.txt"))
            self.max_arity = max([len(edge) for edge in self.dataclass["train_edge"] + self.dataclass["test_edge"] + self.dataclass["valid_edge"]+ self.dataclass["aux_edge"]])
        else:
            self.max_arity = max([len(edge) for edge in self.dataclass["train_edge"] + self.dataclass["test_edge"] + self.dataclass["valid_edge"]])
        
        print("Max_arity:", self.max_arity)
        if self.binary_dataset:
            assert self.max_arity == 2, "Binary dataset must have arity 2"

        self.dataclass["train"] = self.get_torch_tuples(self.dataclass["train_edge"], self.dataclass["train_rel"])
        self.dataclass["test"] = self.get_torch_tuples(self.dataclass["test_edge"], self.dataclass["test_rel"])
        self.dataclass["valid"] = self.get_torch_tuples(self.dataclass["valid_edge"], self.dataclass["valid_rel"])

        if self.name == "JF17K":
            for i in range(2,7):
                test_arity = "test_{}".format(i)
                self.dataclass[test_arity] = self.get_torch_tuples(self.dataclass[test_arity+"_edge"], self.dataclass[test_arity+"_rel"])

        if self.inductive_dataset:
            self.dataclass["aux"] = self.get_torch_tuples(self.dataclass["aux_edge"], self.dataclass["aux_rel"])

        # Graph of torch array we are operating on
        self.dataclass["train_edge_graph"], self.dataclass["train_rel_graph"] = self.dataclass["train"][:,1:].T, self.dataclass["train"][:,0].T

        

        if self.inductive_dataset:
            self.dataclass["test_edge_graph"], self.dataclass["test_rel_graph"] = self.dataclass["aux"][:,1:].T, self.dataclass["aux"][:,0].T
        else:
            self.dataclass["test_edge_graph"], self.dataclass["test_rel_graph"] = self.dataclass["train_edge_graph"], self.dataclass["train_rel_graph"]

        self.batch_index = {"train": 0} # DISABLED test/valid. 

        self.positive_facts_set = list(tuple(t) for t in self.dataclass["train"]) \
                            + list(tuple(t) for t in self.dataclass["test"]) \
                            + list(tuple(t) for t in self.dataclass["valid"])
        
        
        self.dataclass["train_edge"], self.dataclass["train_rel"] = self.dataclass["train"][:,1:].T.to(self.device), self.dataclass["train"][:,0].T.to(self.device)
        self.dataclass["test_edge"], self.dataclass["test_rel"] = self.dataclass["test"][:,1:].T.to(self.device), self.dataclass["test"][:,0].T.to(self.device)
        self.dataclass["valid_edge"], self.dataclass["valid_rel"] = self.dataclass["valid"][:,1:].T.to(self.device), self.dataclass["valid"][:,0].T.to(self.device)

        self.num_relations = self.num_rel()
        print("Number of entity:",self.num_ent()-1)
    ...
    # transform edge_list into torch tuples
    def get_torch_tuple(self, edge, rel):
        return torch.tensor([rel] + list(edge) + [0] * (self.max_arity - len(edge)), dtype=torch.long)

    def get_torch_tuples(self, edges, rels):
        
        return torch.stack([self.get_torch_tuple(edge, rel) for edge, rel in zip(edges, rels)])
    
    def collate(self, data_list):
        return Batch.from_data_list(data_list)


def build_hypergraph_dataset(root, device, name, binary_dataset=False, inductive_dataset=False, alternative_build = False):
    
    # alternative_build=True for Ingram like dataset
    dataset = Hypergraph_Dataset(name, device=device, root = root, binary_dataset = binary_dataset, inductive_dataset = inductive_dataset)  
    train_data = Data(edge_index=dataset.dataclass["train_edge_graph"], edge_type=dataset.dataclass["train_rel_graph"],  num_nodes=dataset.num_ent(),
    ...
    if alternative_build:
        valid_data = Data(edge_index=dataset.dataclass["test_edge_graph"], edge_type=dataset.dataclass["test_rel_graph"], num_nodes=dataset.num_ent(),
                            target_edge_index=dataset.dataclass["valid_edge"], target_edge_type=dataset.dataclass["valid_rel"],
                            num_relations=dataset.num_rel(), max_arity = dataset.max_arity, device = device)
    else:
        valid_data = Data(edge_index=dataset.dataclass["train_edge_graph"], edge_type=dataset.dataclass["train_rel_graph"], num_nodes=dataset.num_ent(),
                            target_edge_index=dataset.dataclass["valid_edge"], target_edge_type=dataset.dataclass["valid_rel"],
                            num_relations=dataset.num_rel(), max_arity = dataset.max_arity, device = device)
    test_data = Data(edge_index=dataset.dataclass["test_edge_graph"], edge_type=dataset.dataclass["test_rel_graph"], num_nodes=dataset.num_ent(),
    ...
    collated_result = dataset.collate([train_data, valid_data, test_data])
    return collated_result


def JF17K(root,device):
    return build_hypergraph_dataset(root, device, name="JF17K")

def FBAUTO(root,device):
    return build_hypergraph_dataset(root, device, name="FB-AUTO")

def Wikipeople(root,device):
    return build_hypergraph_dataset(root, device, name="Wikipeople")
...
def JF100(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="JF-100", inductive_dataset = True, alternative_build=True)

def JF75(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="JF-75", inductive_dataset = True, alternative_build=True)

def JF50(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="JF-50", inductive_dataset = True, alternative_build=True)

def JF25(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="JF-25", inductive_dataset = True, alternative_build=True)

def WPIND(root, device, version=None):
    return build_hypergraph_dataset(root, device, name="WP-IND", inductive_dataset = True)

def WP100(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="WP-100", inductive_dataset = True, alternative_build=True)

def WP75(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="WP-75", inductive_dataset = True, alternative_build=True)

def WP50(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="WP-50", inductive_dataset = True, alternative_build=True)

def WP25(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="WP-25", inductive_dataset = True, alternative_build=True)

def MFBIND(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="MFB-IND", inductive_dataset = True)

def MFB100(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="MFB-100", inductive_dataset = True, alternative_build=True)

def MFB75(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="MFB-75", inductive_dataset = True, alternative_build=True)

def MFB50(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="MFB-50", inductive_dataset = True, alternative_build=True)

def MFB25(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="MFB-25", inductive_dataset = True, alternative_build=True)

def WD100(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="WD-100", inductive_dataset = True, alternative_build=True)

def WD75(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="WD-75", inductive_dataset = True, alternative_build=True)

def WD50(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="WD-50", inductive_dataset = True, alternative_build=True)

def WD25(root,device, version=None):
    return build_hypergraph_dataset(root, device, name="WD-25", inductive_dataset = True, alternative_build=True)


class GrailInductiveDataset(InMemoryDataset):

    def __init__(self, device, root, version, transform=None, 
                 pre_transform=T.Compose([build_weighted_graph]), 
    ...
    def process(self):
        ...
        # Hypergraph: Modify to add the virtual 0-th node.
        edge_index += 1
        ...
        # we do not need flipped triplets if we are using hcnets
        ...
        train_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=len(inv_train_entity_vocab)+1,
                          target_edge_index=edge_index[:, train_slice], target_edge_type=edge_type[train_slice], num_relations=num_relations, max_arity = 2, device = self.device)
        valid_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=len(inv_train_entity_vocab)+1,
                          target_edge_index=edge_index[:, valid_slice], target_edge_type=edge_type[valid_slice], num_relations=num_relations, max_arity = 2,device = self.device)
        test_data = Data(edge_index=test_fact_index, edge_type=test_fact_type, num_nodes=len(inv_test_entity_vocab)+1,
                         target_edge_index=edge_index[:, test_slice], target_edge_type=edge_type[test_slice], num_relations=num_relations, max_arity = 2, device = self.device)
...
def FB15k237(root, device):
    ...
    # Hypergraph: remove added inverse edges
    data.num_relations = dataset.num_relations // 2
    data.edge_index = data.edge_index[:, :data.edge_index.size(1) // 2]
    data.edge_type = data.edge_type[:data.edge_type.size(0) // 2]

    # Hypergraph: Modify to add the virtual 0-th node.
    data.edge_index += 1
    data.train_edge_index += 1
    data.valid_edge_index += 1
    ...
    train_data = Data(edge_index=data.edge_index, edge_type=data.edge_type, num_nodes=data.num_nodes,
                        ...
                        num_relations=dataset.num_relations, max_arity = 2, device = device)
...
def WN18RR(root, device):
    ...
    train_data = Data(edge_index=edge_index, edge_type=edge_type, num_nodes=num_nodes,
                        ...
                        num_relations=num_relations, max_arity = 2, device = device)
...
class TransductiveDataset(InMemoryDataset):
    ...
    def __init__(self, root, device, transform=None, pre_transform=T.Compose([build_weighted_graph]), **kwargs):
        self.device = device
        super().__init__(root, transform, pre_transform)
        self.data, self.slices = torch.load(self.processed_paths[0])
    ...
    def process(self):
        ...
        valid_edges += 1
        test_edges += 1
        train_target_edges += 1
        num_node += 1

        train_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                          target_edge_index=train_target_edges, target_edge_type=train_target_etypes, num_relations=num_relations, max_arity = 2, device = self.device)
        valid_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                          target_edge_index=valid_edges, target_edge_type=valid_etypes, num_relations=num_relations, max_arity = 2, device = self.device)
        test_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                         target_edge_index=test_edges, target_edge_type=test_etypes, num_relations=num_relations, max_arity = 2, device = self.device)
...
class NELL995(TransductiveDataset):
    ...
    def process(self):
        ...
        train_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                          target_edge_index=train_target_edges, target_edge_type=train_target_etypes, max_arity = 2, num_relations=num_relations, device = self.device)
        valid_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                          target_edge_index=valid_edges, target_edge_type=valid_etypes, max_arity = 2,  num_relations=num_relations, device = self.device)
        test_data = Data(edge_index=train_edges, edge_type=train_etypes, num_nodes=num_node,
                         target_edge_index=test_edges, target_edge_type=test_etypes, max_arity = 2, num_relations=num_relations, device = self.device)
...
class InductiveDataset(InMemoryDataset):
    ...
    def __init__(self, root, version, device, transform=None, pre_transform=T.Compose([build_weighted_graph]),  **kwargs):
    ...
    def process(self):
        ...
        # Disable the inverse edges
        train_fact_index = train_target_edges
        train_fact_type = train_target_etypes
        ...
        # Hypergraph: Modify to add the virtual 0-th node.
        num_train_nodes += 1
        inference_num_nodes += 1
        train_target_edges += 1
        inf_edges += 1


        train_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=num_train_nodes, max_arity = 2,
                          target_edge_index=train_target_edges, target_edge_type=train_target_etypes, num_relations=num_train_rels, device = self.device)
        valid_data = Data(edge_index=inf_edges if self.valid_on_inf else train_fact_index, 
                          ...
                          target_edge_index=inf_valid_edges[:, :2].T + 1, 
                          target_edge_type=inf_valid_edges[:, 2], max_arity = 2,
                          num_relations=inference_num_rels if self.valid_on_inf else num_train_rels, device = self.device)
        test_data = Data(edge_index=inf_edges, edge_type=inf_etypes, num_nodes=inference_num_nodes,
                         target_edge_index=inf_test_edges[:, :2].T + 1, target_edge_type=inf_test_edges[:, 2], num_relations=inference_num_rels, max_arity = 2, device = self.device)
...
class HM(InductiveDataset):
    ...
    def process(self):
        ...
        train_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=num_train_nodes,
                          target_edge_index=train_target_edges, target_edge_type=train_target_etypes, max_arity = 2, num_relations=num_train_rels, device = self.device)
        valid_data = Data(edge_index=train_fact_index, 
                          edge_type=train_fact_type, 
                          num_nodes=valid_res["num_node"] + 1,  # the only fix in this function
                          max_arity = 2,
                          target_edge_index=inf_valid_edges[:, :2].T + 1, 
...
class MTDEAInductive(InductiveDataset):
    ...
    def process(self):
        ...
        train_data = Data(edge_index=train_fact_index, edge_type=train_fact_type, num_nodes=num_train_nodes,
                        target_edge_index=train_target_edges, target_edge_type=train_target_etypes, num_relations=num_train_rels, max_arity = 2, device = self.device)
        valid_data = Data(edge_index=train_fact_index, 
                        edge_type=train_fact_type, 
                        num_nodes=valid_res["num_node"] + 1,  # the 2nd fix in this function
                        target_edge_index=inf_valid_edges[:, :2].T + 1, 
                        target_edge_type=inf_valid_edges[:, 2], 
                        num_relations=inference_num_rels if self.valid_on_inf else num_train_rels, max_arity = 2, device = self.device)
        test_data = Data(edge_index=inf_edges, edge_type=inf_etypes, num_nodes=inference_num_nodes,
                        target_edge_index=inf_test_edges[:, :2].T + 1, target_edge_type=inf_test_edges[:, 2], num_relations=inference_num_rels, max_arity = 2, device = self.device)
...
# a joint dataset for pre-training HYPER on several graphs
